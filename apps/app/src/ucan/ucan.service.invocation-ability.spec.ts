...
const SERVICE_URL = 'https://subscriptions.example';
...
function makeConfig(values: Record<string, unknown>): ConfigService<ENV> {
  return {
...
describe('UcanService — invocation ability', () => {
  ...
  beforeEach(async () => {
    ...
    vi.spyOn(svc, 'resolveServiceDid').mockResolvedValue(SERVICE_DID);
  ...
  it('claims the requested ability rather than the wildcard', async () => {
    await svc.createServiceInvocation(
      SERVICE_URL,
      USER_DID,
      'ixo:subscriptions',
      {
  ...
  it("defaults to '*' so existing callers are unchanged", async () => {
    await svc.createServiceInvocation(SERVICE_URL, USER_DID, 'ixo:memory');
